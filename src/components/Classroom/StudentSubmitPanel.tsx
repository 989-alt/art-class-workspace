...
const L = {
    title: '내 작품 제출',
    help: '색칠을 완료한 후 사진을 촬영해 제출하면 선생님이 검수한 뒤 학급 전시장에 올라갑니다.',
    pick: '📸 사진 선택',
    uploading: '업로드 중...',
    submitted: '✅ 제출 완료! 선생님 검수 대기중',
    pendingHint: '검수가 끝날 때까지 잠시만 기다려 주세요. 다시 제출하려면 아래 버튼을 눌러 주세요.',
    retake: '🔄 다시 제출',
    altPreview: '내가 제출한 작품',
...
export default function StudentSubmitPanel({
    ...
    const {
        ...
        error,
        submit,
    } = useStudentSubmission(configured ? assignmentId : null, studentToken);
            ...
            {submission ? (
                <div className="submit-panel__submitted">
                    <img
                        ...
                        alt={L.altPreview}
                    />
                    <p className="submit-panel__status" role="status">
                        {L.submitted}
                    </p>
                    <p className="submit-panel__locked-hint">{L.pendingHint}</p>
                    <button
                        type="button"
